...
function scene:create( event )
    ...
    local short = display.newImage("Images/short.png",160,50,50,50)
    sceneGroup:insert(short)
    short.alpha = 1 
    
    local img_text = display.newText( "Short", _CX,165,               "Fonts/chapaza/Chapaza", 48 )
    img_text:setTextColor(.3,1,.2)
    img_text.alpha = 1
    sceneGroup:insert(img_text)
    
    --transition.to( short, { time=1900, delay=1300, alpha=1.0,} )
    --transition.to( img_text, { time=1900, delay=1300, alpha=1.0,} )
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
        ...
        transition.to( button4, { time=3000, alpha=1, onComplete=completeListener } )
        ...
        transition.to( button_text1, { time=1000, alpha=1, onComplete=completeListener } )
